...
/*
 * 複合式排序 (Composite Sort)：本作業的最終目標核心。
...
int main() {
    ...
    for (int n : n_values) {
        // 智慧型重複次數微調（智慧型防秒殺機制）：
        // 為了達到投影片 1% 精確度要求，n 越小，重複排序越多次來拉長總時間，消除硬體計時誤差
        int reps = 20;
        if (n <= 100) reps = 500;
    ...
    // =========================================================================
    // 格式化輸出區塊：精確對齊並復刻規範版面 (寬度防爆錯位設計)
    // =========================================================================
    cout << "========================================================================================\n";
    cout << "                    演算法實驗：最壞情況執行時間（單位：毫秒）\n";
    cout << "========================================================================================\n";

    // 靠左對齊，並配置嚴格的 18 個字元固定欄位寬度（setw(18)）
    cout << left << setw(10) << "n"
        << setw(18) << "Insertion"
        << setw(18) << "Quick"
        << setw(18) << "Merge"
