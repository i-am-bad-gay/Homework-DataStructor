...
本專案採用自底向上的開發方式。
首先完成最基本的節點結構，再逐步建立鏈結串列容器與 Iterator，
確認基礎資料結構正確後，才進行多項式相關功能的實作。

在開發過程中，每完成一個模組即進行測試，
例如先測試 Chain 與 ChainIterator 的走訪正確性，
再測試 Polynomial 的插入與加法運算，
藉此降低除錯難度並確保程式穩定性。

### 循環鏈結串列設計之優點

本作業在 Polynomial 中採用循環鏈結串列（Circular Linked List），
並設置 header node 作為管理節點。
此設計可避免在插入與走訪時頻繁判斷 nullptr，
使程式邏輯更為簡潔，也降低邊界錯誤發生的機率。
